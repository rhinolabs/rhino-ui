...
const generateShorthandProperties = (utility, prop, variation) => {
  // CSS Attribute specifies the variation. E.G: 'margin-bottom: <value>'
  const single = ["top", "right", "bottom", "left"];
  // CSS Attribute applied to multiple sides of an element: E.G: 'margin: <value> <value>'
  const compound = ["", "h", "v"];
  ...
  if (single.includes(variation)) {
    ...
    if (variation === "") {
      ...
      if (property === "border") {
        output = `${property}-left-width: ${value};
      ...
      if (property === "border") {
        output = `${property}-top-width: ${value};
...
const utilityClass = {
  ...
  formatter(dictionary) {
    ...
    const sortedBreakpoints = breakpoints.sort(
      (a, b) => parseInt(a.original.value, 10) - parseInt(b.original.value, 10),
    );

...
import { color } from '@rhino-ui/design-tokens/build/json/variables-color.json';
import { size } from '@rhino-ui/design-tokens/build/js/variables-size';
import { asset } from '@rhino-ui/design-tokens/build/js/variables-asset';
...
[RhinoUI Design Tokens](https://github.com/rhinolabs/rhino-ui/tree/main/packages/design-tokens) are available as an [npm package](https://www.npmjs.com/package/@rhino-ui/design-tokens) so that you can create a unified product experience for your platform of choice.
...
RhinoUI Components follow a "mobile-first" approach, where styles are set with mobile devices considered first. Then styling adjustments are made as each minimum viewport width &mdash; the breakpoint &mdash; is reached.

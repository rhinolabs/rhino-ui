...
import { TimePickerNative } from "../components/time-picker-native/time-picker-native";
...
<Canvas>
  <Story name="Custom Theme Form">
    {() => {
      ...
      return (
        <Box childGap="lg" background="grey-50" padding="lg" display="block" style={isThemed ? customTheme : null}>
          <Toggle label="Apply Custom Theme" isChecked={isThemed} onChange={(e) => setIsThemed(e.target.checked)} />
          <TextInput
            id="demo"

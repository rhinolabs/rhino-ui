...
<Canvas>
  <Story name="Date Range">
    {() => {
      ...
      return (
        <Box gap="md">
          <DatePicker onChange={setDate} selected={startDate} startDate={startDate} endDate={endDate} selectsRange />
          <p>{`Selected Date Range: ${startDate && startDate.toISOString()} - ${endDate && endDate.toISOString()}`}</p>
        </Box>
      );
    }}
  </Story>
...
<Canvas>
  <Story name="Open on Date">
    {() => {
      ...
      return (
        <Box gap="md">
          <DatePicker onChange={setStartDate} selected={startDate} openToDate={new Date("1993/09/28")} />
          <p>Selected Date: {startDate && startDate.toISOString()}</p>
        </Box>
      );
    }}
        ...
        <Box gap="md">
          ...
          >
            <Box margin="md 0" fontWeight="bold">
              Here we've overwritten base token values so that the DatePicker inherits new values.
            </Box>
            <DatePicker onChange={setSelectedDate} selected={selectedDate} />
          </div>
          <div

    ...
    {() => {
      ...
      return (
        <Box display="inline-block">
          ...
          >
            <Button onMouseOver={openPopover} onMouseOut={closePopover} variant="light">
              Hover Me
            </Button>
          </Popover>
        </Box>
...
<Canvas withSource="open">
  ...
  >
    {() => {
      const { isOpen: isPopoverOpen, handleClose: closePopover, handleToggle: togglePopover } = useOpenClose();
      const popoverContent = (
        <>

import { createElement, cloneElement, FC, forwardRef, ReactNode, ReactElement, CSSProperties, Children } from "react";
...
import { getDimensionCss } from "../../lib/get-dimension-css";
...
export const Box: FC<BoxProps> = forwardRef(
  (
    ...
    const getStatefulClasses = (stateKey: "hover" | "focus", values: BoxProps["hover" | "hover"]) =>
      values // eslint-disable-line
        ? Object.entries(values).map(
            ([key, value]) =>
              cssPropertyMap[key].transformer(
                `${stateKey}:${cssPropertyMap[key as keyof BoxProps["focus" | "hover"]].classPrefix}`,
                value,

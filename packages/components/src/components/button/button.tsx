...
export type AnchorButtonProps = { as: "a" } & BaseButtonProps &
  Omit<React.DetailedHTMLProps<AnchorHTMLAttributes<HTMLAnchorElement>, HTMLAnchorElement>, "ref">;

export type NormalButtonProps = { as?: "button" } & BaseButtonProps &
  Omit<React.DetailedHTMLProps<ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement>, "ref">;
  ...
  (
    ...
    const buttonContent =
      iconPrefix || iconSuffix ? (
        <Box display="inline-flex" direction="row" alignItems="center" gap={size === "xs" ? "2xs" : "xs"}>
          {isLoading && <Spinner variant={getSpinnerVariant()} className={styles["spinner-wrapper"]} />}
          {iconPrefix && (
            <Icon

...
import { OptionTile } from "../option-tile/option-tile";
import { InputValidationMessage } from "../input-validation-message/input-validation-message";
import styles from "./option-tile-group.module.scss";
...
interface Option {
  ...
  render?: (option: { id: string; value: string; label: string; disabled?: boolean }) => React.ReactNode;
}
...
export const OptionTileGroup: ForwardRefExoticComponent<OptionTileGroupProps> = forwardRef<
  HTMLDivElement,
...
>(
  (
    ...
    return (
      <Box ref={ref} className={className} width={isFullWidth ? "100" : undefined} {...restProps}>
        <Box
          role="group"
          childGap="md"
          borderWidth="0"
        ...
        >
          {(title || description) && (
            ...
            >
              ...
              {description && (
                <Box margin="xs 0 0 0" fontWeight="regular">
                  {description}
                </Box>
              )}
            </Box>

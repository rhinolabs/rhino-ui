...
export const generatePages = (
  ...
  if (pageTotal > pages[pages.length - 1]?.pageNumber) {
    const secondToLastPage =
      pageTotal !== activePage + numberOfPagesDisplayed ? activePage + numberOfPagesDisplayed : pageTotal - 1;
  ...
  if (activePage > numberOfPagesDisplayed) {
    ...
    pages.unshift({ pageNumber: 1, isPage: true }, { pageNumber: threeDotsPage, isPage: false });
  }
...
export const generatePageRange = (numberOfPagesDisplayed: number, pageTotal: number): number =>

...
export const FileUpload: FC<FileUploadProps> = React.forwardRef<HTMLDivElement, FileUploadProps>(
  (
    ...
    return (
      <Box display="inline-block" className={className} width={fullWidth ? "100" : undefined} ref={ref} {...restProps}>
        ...
        >
          ...
          >
            ...
            >
              ...
              {buttonText && <Box data-testid="file-upload__upload-text">{buttonText}</Box>}
        ...
        {error && error !== true && <InputValidationMessage size={messageFontSize()}>{error}</InputValidationMessage>}
        {renderFiles()}
        {files && onClearFiles && (
